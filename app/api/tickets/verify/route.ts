...
export async function GET(req: Request) {
  try {
    ...
    // 1. Recherche par code direct de billet
    if (code) {
    ...
    // 2. Traitement après paiement Stripe (session_id)
    if (sessionId) {
      ...
      // Si le billet n'est pas encore en base, récupération directe depuis Stripe
      ...
      if (session.payment_status === 'paid') {
        ...
        const ticketType = session.metadata?.ticket_type || 'PASS VIP OFFICIEL';
        ...
        // Envoi automatique et immédiat de l'email avec le QR Code
        if (customerEmail) {
          ...
          try {
            ...
            console.log('✅ Email avec billet envoyé avec succès.');
          } catch (emailErr) {
